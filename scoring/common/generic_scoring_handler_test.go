...
func TestGenericScoringHandler(t *testing.T) {
	requestData := []byte(`
	{
		"newExposureSummary":
		{
	...
	status, response, err := GenericScoringHandler(body)
	assert.Equal(t, 200, status)
	assert.Equal(t, nil, err)
	assert.Equal(t, expectedResponse, response)
}

func TestGenericEmptyBody(t *testing.T) {
	// Test that we get an error on an empty body.
	status, response, err := GenericScoringHandler("")
	assert.Equal(t, 400, status)
	assert.Equal(t, ErrRequestBodyEmpty, err)
	assert.Equal(t, "", response)
}

func TestGenericMalformedRequest(t *testing.T) {
	status, response, err := GenericScoringHandler("i am not a valid JSON string")
	assert.Equal(t, 400, status)
